...
**本书全部内容由 AI（ZCode / GLM 模型）生成**：从全书结构、各章正文到打包实例的逐行注释，均由 AI 撰写；人类作者的角色是提出需求、审阅与修改意见（如增补 home-manager 一章、调整结构等），并做最终把关。请读者据此判断内容的可靠程度：尽管写作过程中尽量以官方手册、NixOS Wiki 与 nixpkgs 真实源码为依据并标注来源，AI 生成内容仍可能存在事实错误与过时细节，使用前请与官方文档交叉验证，发现问题欢迎提 issue。
...
以上作废：本书实为**个人自用**的学习笔记与参考手册，顺带公开分享。
...
本书为 AI 生成、人工审阅修订的学习资料，内容参考并整理自 Nix / NixOS 官方手册、NixOS Wiki、nix.dev、nixpkgs 源码及社区公开资料（正文中尽可能标注了来源），仅供学习与研究参考，不作商业用途。书中引用的 nixpkgs 源码版权归其贡献者所有（MIT 许可证）。如有侵权或来源标注遗漏，请提 issue 或联系作者，将及时处理。在未补充正式许可证之前，本书内容默认保留所有权利，请勿整体转载。
